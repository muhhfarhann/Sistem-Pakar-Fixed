...
if (isset($_POST["submit"])) {
    ...
    if (!isset($_POST["check"])) {
        $error = true;
    } elseif (updateKerusakan($_POST) > 0) {
        echo "
        <script>
    ...
    header('Location: dataKerusakan.php');
    exit;
}
...
<body>
    ...
    <main class="main" id="main">
        <h2 class="text-center text-success">Update Kerusakan</h2>
        <?php if ($error) : ?>
        ...
        <div class="container border border-dark shadow-lg w-50">
            <div class="row">
                <form action="" method="post" class="row" data-sb-form-api-token="your-token-here">
                    <div class="col-md-12">
                        <input type="hidden" name="kodeKerusakan" value="<?= htmlspecialchars($result["kodeKerusakan"]); ?>">
                        <label for="nama" class="form-label">Nama Kerusakan</label>
                        <input type="text" class="form-control w-100" id="nama" value="<?= htmlspecialchars($result["namaKerusakan"]); ?>" name="namaKerusakan" autocomplete="off" required>
