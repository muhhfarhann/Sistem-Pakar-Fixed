...
$query1 = "SELECT * FROM kerusakan";

$result = query($query);

$dataKerusakan = query($query1);
...
<body>
    ...
    <?php
        ...
        $is_home_page = ($current_page == 'insertSolusi.php');
    ?>
    ...
    <section class="form" id="form">
        <div class="container">
            <h4>INSERT DATA SOLUSI</h4>
            <?php if($error) : ?>
                <p style="font-style: italic; color: red; text-align: center;">Please push checkbox</p>
            <?php endif; ?>
            <form action="" method="post">
                <div class="row">
                    <select name="kodeKerusakan" id="">
                        <option value="" class="selected" selected disabled>Pilih Kerusakan Untuk Solusi</option>
                        <?php foreach($dataKerusakan as $data) : ?>
                            <option value="<?= $data["kodeKerusakan"]; ?>">Kode Kerusakan <?= $data["kodeKerusakan"]; ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="row">
                    <label for="solusi">Solusi</label>
                    <input type="text" name="solusi" id="solusi" autocomplete="off">
                </div>
                <div class="row check">
